...
module Fluent
  module Plugin
    class KubernetesObjectsInput < Fluent::Plugin::Input
      ...
      desc "If `insecure_ssl` is set to `true`, it won't
            verify apiserver's certificate."
      config_param :insecure_ssl, :bool, default: false

      desc 'Path to the file contains the API token. By
            default it reads from the file "token" in the `secret_dir`.'
      config_param :bearer_token_file, :string, default: nil

      desc "Path of the location where pod's service account's
      ...
      config_section :watch, required: false, init: false,
                     multi: true, param_name: :watch_objects do
        desc 'The name of the resource, e.g. "events".'
        config_param :resource_name, :string

        desc 'The namespace of the resource, it watches
              all namespaces if not set.'
        config_param :namespace, :string, default: nil

        desc 'The name of the entity to watch, use this
              to watch only one entity.'
      ...
      def configure(conf)
        ...
        raise Fluent::ConfigError,
              'At least one <pull> or <watch> is required,
               but found none.' if @pull_objects.empty? && @watch_objects.empty?
      ...
      def initialize_client
        ...
        ssl_options = {
          ...
          ca_file:
              @ca_file,
          verify_ssl:
              @insecure_ssl ? OpenSSL::SSL::VERIFY_NONE : OpenSSL::SSL::VERIFY_PEER
        }
        ...
        begin
          ...
          raise Fluent::ConfigError,
                "Invalid Kubernetes API #{@api_version} endpoint
                 #{@kubernetes_url}: #{kube_error.message}"
        end
      end
      ...
      def create_pull_thread(conf)
        ...
        thread_create :"pull_#{resource_name}" do
          ...
          while thread_current_running?
            ...
            # kubeclient will create one open struct object for each item in the
            # response, but this is totally unecessary in this plugin,
            # thus we use as: :raw.
